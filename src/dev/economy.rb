...
module Bot::Economy
  ...
  def self.get_user_checkin_value(user_id)
    ...
    if role_yaml_id == nil
      raise "Unexpected role ID received, there may be a new role that needs to be accounted for by checkin!"
    end
        ...
        owner.user.dm.send_embed do |embed|
          ...
          removed_items.each do |item|
            ...
            if name_override[item.entry_id] != nil
              item_ui_name = (name_override[item.entry_id]).to_s
            end
  ...
  command :shop do |event|
    ...
    event.send_embed do |embed|
      ...
      types.each do |type_id, type_name|
        ...
        cost_str = pl(type_cost, "Starbuck").to_s
